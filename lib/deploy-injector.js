...
export async function injectDeployConfig(projectDetails) {
  ...
  try {
    ...
    await addDeploymentSpecificFiles(projectPath, deployTarget);
...
async function addGitHubActions(projectPath, projectDetails) {
  ...
  const workflowContent = template({
    ...
    gcpProjectId: deployConfig.gcpProjectId || '',
    gcpRegion: deployConfig.gcpRegion || 'us-central1'
  });
  
  // Write workflow file
  await writeFile(join(workflowDir, 'deploy.yml'), workflowContent);
...
This project is configured for deployment to ${deployTarget}.
...
async function addVercelConfig(projectPath, projectDetails) {
  ...
  const vercelConfig = {
    buildCommand: framework === 'nextjs' ? null : 'npm run build',
